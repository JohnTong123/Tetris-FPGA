...
//u_int16 XPAR_SPI_USB_DEVICE_ID = ;
...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    if (Status != XST_SUCCESS) {
        xil_printf("MAXreg_wr failed with status %d\r\n", Status);
    }
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    if (Status != XST_SUCCESS) {
        xil_printf("MAXbytes_wr failed with status %d\r\n", Status);
    }
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    Status = XSpi_Transfer(&SpiInstance, SendBuf, RecvBuf, 2);
    if (Status != XST_SUCCESS) {
        xil_printf("Erro: MAXreg_rd failed with status %d\r\n", Status);
        val = 0;
    } else {
        val = RecvBuf[1];
    }
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    Status = XSpi_Transfer(&SpiInstance, SendBuf, RecvBuf, nbytes + 1);
    if (Status != XST_SUCCESS) {
        xil_printf("Error: MAXbytes_rd failed with status %d\r\n", Status);
    } else {
        for (int i = 0; i < nbytes; i++) {
            data[i] = RecvBuf[i + 1];
        }
...
//	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_TIMER_USB_AXI_DEVICE_ID);
	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_AXI_TIMER_0_DEVICE_ID);
	if (Status != XST_SUCCESS) {
